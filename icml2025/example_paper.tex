...
\begin{enumerate}
    ...
    \item We show that compiler optimization can substantially improve performance, reducing runtime by up to 3.6x and compile overhead by up to 1.8x on Laplacian benchmarks.
...
%0 = stablehlo.constant dense<1.280183e+01> : tensor<f32>
\end{verbatim}
}
...
{\scriptsize
\begin{verbatim}
%cst = stablehlo.constant dense<3.00e+00> : tensor<f32>
%0 = chlo.lgamma %cst : tensor<f32> -> tensor<f32>
%1 = stablehlo.exponential %0 : tensor<f32>
%2 = stablehlo.broadcast_in_dim %1, dims = []
       : (tensor<f32>) -> tensor<256x3xf32>
\end{verbatim}
}
...
%z = stablehlo.constant dense<0.00e+00> : tensor<f32>
%0 = stablehlo.broadcast_in_dim %z, dims = []
       : (tensor<f32>) -> tensor<256x3x256xf32>
%1 = stablehlo.add %0, %x : tensor<256x3x256xf32>
\end{verbatim}
}
...
\section{Evaluation}
In this section, we evaluate our approach on two benchmarks: 


\subsection{MLP Laplacian}
\subsection{Physics-Informed NN for Poisson's Equation}
\subsection{}
...
\subsection{Hessian Diagonal by JVP-of-Grad: \texttt{MLP\_LAPLACIAN\_JVP\_GRAD}}

The second method computes Hessian-vector products using a JVP of the gradient:
\begin{verbatim}
grad_fn = jax.grad(scalar_fn)
...
\[
  ...
  \nabla^2 f(x)v.
\]
Using a basis direction $e_i$ gives
\[
  e_i^\top \nabla^2 f(x)e_i = H_{ii}(x).
\]
Therefore, running this computation for each basis vector and summing the results computes the Laplacian.

Unlike the dense Hessian method, this approach does not materialize the full Hessian. Instead, it produces repeated Hessian-vector-product computations, one per basis direction:
\[
  e_0^\top H e_0
  +
  e_1^\top H e_1
  ...
  e_2^\top H e_2.
\]
This exposes repeated derivative subgraphs, basis-vector constants, zero tangents, dot products, broadcasts, and reshapes. These repeated structures can create opportunities for common subexpression elimination and canonicalization.

\subsection{Repeated First-Order AD Composition: \texttt{MLP\_LAPLACIAN\_AUTO}}

The \texttt{MLP\_LAPLACIAN\_AUTO} method also computes higher-order derivatives by composing first-order AD transformations. Examples include forward-over-reverse, reverse-over-forward, and reverse-over-reverse AD. These methods are mathematically equivalent for smooth functions, but they may lower to different StableHLO programs.

For an MLP layer
\[
  h_{\ell+1} = \sigma(W_\ell h_\ell + b_\ell),
\]
forward-mode structure introduces tangent computations such as
\[
  \dot{z}_\ell = W_\ell \dot{h}_\ell,
  \qquad
  \dot{h}_{\ell+1} = \sigma'(z_\ell)\dot{z}_\ell.
\]
Reverse-mode structure introduces cotangent computations such as
\[
  \bar{h}_\ell = W_\ell^\top \bar{z}_\ell.
\]

Thus, repeated first-order AD can expose explicit linear-algebraic derivative structure: tangent propagation, cotangent propagation, transposed weight matrices, activation derivatives, and repeated primal computations. Although this can make the initial IR larger, it may also make optimization opportunities more visible to standard compiler passes.

\subsection{Taylor-Mode Higher-Order AD: \texttt{MLP\_LAPLACIAN\_JET}}

The \texttt{MLP\_LAPLACIAN\_JET} method uses Taylor-mode AD. For each basis direction $e_i$, it considers the one-dimensional curve
\[
  g_i(t) = f(x + t e_i).
\]
Then
...
Instead of repeatedly composing first-order AD operators, Taylor-mode propagates truncated Taylor coefficients through the program. For a second-order computation, an intermediate value is represented by coefficients such as
\[
  (a_0, a_1, a_2),
\]
where $a_0$ is the primal value, $a_1$ is the first directional derivative, and $a_2$ is the second directional derivative.

For example, if
\[
  y = \sigma(z),
\]
Taylor-mode propagates equations of the form
\[
  y_0 = \sigma(z_0),
  \qquad
  y_1 = \sigma'(z_0)z_1,
  \qquad
  y_2 = \sigma''(z_0)z_1^2 + \sigma'(z_0)z_2.
\]
This exposes coefficient-wise propagation through the primal program rather than explicit Hessian tensors or reverse-mode cotangent computations.



A larger repeated-composition program may contain more redundant structure for common subexpression elimination, while a compact Taylor-mode program may have less obvious redundancy. This motivates comparing higher-order AD methods not only by their source-level formulation, but also by the StableHLO IR they produce and how effectively that IR can be optimized.
